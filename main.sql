...
('MATH', 'Calculus I', 'MATH101', 'Spring', 2024),
('ENG', 'Introduction to Literature', 'ENG101', 'Spring', 2024);
...
(2, 1),
(3, 1),
(4, 1);  -- Enroll Emily in Introduction to Computer Science
...
INSERT INTO Scores (student_id, assignment_id, score)
VALUES
(4, 1, 78), -- Full participation
(4, 2, 93),  -- Score for Homework 1
(4, 3, 45);  -- Score for Midterm
...
JOIN
    Courses c ON e.course_id = c.course_id
JOIN
    Assignments a ON a.category_id IN (SELECT category_id FROM Categories WHERE course_id = c.course_id)
JOIN
    Scores sc ON s.student_id = sc.student_id AND a.assignment_id = sc.assignment_id
WHERE
    c.course_name = 'Introduction to Computer Science';
...
TotalScores AS (
    SELECT
        sc.student_id,
        a.category_id,
        SUM(sc.score) - COALESCE(MIN(ls.lowest_score), 0) AS adjusted_score, -- Use MIN() function to aggregate lowest_score
